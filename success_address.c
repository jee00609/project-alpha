...
int main()
{
	...
	char search2[20];//2차 검색변수
	char contrast[20];//등록된 이름을 2차 검색변수와 비교할 아이다.
	...
	while(1)
	{
		...
		*/
		...
		if(select==5)//txt저장
		{
			...
			for(i=0;i<o;i++)
			{
				fprintf(fp,"이름: %s\t집전화: %s\t개인전화: %s\t메모:%s\n",add[i].name,add[i].hnum,add[i].pnum,add[i].memo);
				/*fscanf(fp,"이름: %s\t집전화: %s\t개인전화: %s\t메모:%s\n",add[i].name,add[i].hnum,add[i].pnum,add[i].memo);
					로 쓰면 txt 저장했다고 좋아라 한다음 열면 여백의 아름다움이 너를 반길거야
					fscanf를 쓰게 되면 txt파일에서 써있는 것을 가져와 나타내는 거거든 그러니까 w파일로 열기때문에 내용이 공백이 되고
					그걸 또 c에 불러오려고 하니까 걍 공백이 되는거지!*/

				//학기 종강하고 나서 다시 만들더니 정말 완벽하게 완성한 나의 마음은 족같지만 기쁘기도 하군요!!!
				//fprintf와 fscanf의 차이를 아직까지도 모른다고 하면 바보겠지?
			}
